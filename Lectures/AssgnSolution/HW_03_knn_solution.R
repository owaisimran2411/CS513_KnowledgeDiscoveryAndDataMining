...
#e.	Measure the performance of knn
  ...
  factor(fit)
  library(caret)
  confusionMatrix(factor(fit), factor(test$Class), mode = "everything")
  confusionMatrix(factor(fit), factor(test$Class), mode = "everything", positive="4")
  length(fit)
  results<-data.frame(cbind(fit,test[,11] ))
